...
type FormErrors = Partial<Record<'name' | 'description' | 'phone' | 'location', string>>;

const initialFormState = (restaurant: ReturnType<typeof useSellerStore.getState>['myRestaurant']) => ({
    name: restaurant?.name ?? '',
    description: restaurant?.description ?? '',
    phone: restaurant?.phone?.toString() ?? '',
    latitude: restaurant?.autoLocation.coordinates[1] ?? 0,
    longitude: restaurant?.autoLocation.coordinates[0] ?? 0,
    formattedAddress: restaurant?.autoLocation.formattedAddress ?? '',
});

const UpdateRestaurantDialogBox = ({ visible, onClose }: Props) => {
    const { myRestaurant } = useSellerStore();
    const [form, setForm] = useState(initialFormState(myRestaurant));
    ...
    useEffect(() => {
        ...
        setForm((current) => ({
            ...current,
            latitude: myRestaurant?.autoLocation.coordinates[0] ?? current.latitude,
            longitude: myRestaurant?.autoLocation.coordinates[1] ?? current.longitude,
            formattedAddress: myRestaurant?.autoLocation.formattedAddress ?? current.formattedAddress,
        }));
    }, [location, visible]);
    ...
    const { updateRestaurant } = useSellerStore();
    ...
    const validate = () => {
        ...
        if (!myRestaurant?.autoLocation?.formattedAddress || !myRestaurant.autoLocation.coordinates[0] || !myRestaurant.autoLocation.coordinates[1]) {
    ...
    const handleClose = () => {
        setErrors({});
        setForm(initialFormState(myRestaurant));
        onClose();
    };

    const hasLocation = Boolean(myRestaurant?.autoLocation?.formattedAddress && myRestaurant?.autoLocation?.coordinates[0] && myRestaurant?.autoLocation?.coordinates[1]);
                                    ...
                                    <View className="flex-row items-start mb-3">
                                        ...
                                        <View className="flex-1 ml-2">
                                            ...
                                            <Text className="mt-1 text-xs leading-5" style={{ color: AUTH_COLORS.textSubtle }}>
                                                {myRestaurant?.autoLocation?.formattedAddress || 'No saved location found.'}
                                            </Text>
                                        </View>
                                    </View>
                                    ...
                                    <View className="flex-row flex-wrap gap-2">
                                        <View className="px-3 py-2 border rounded-full" style={{ backgroundColor: AUTH_COLORS.chip, borderColor: AUTH_COLORS.chipBorder }}>
                                            <Text className="text-xs font-semibold" style={{ color: AUTH_COLORS.textPrimary }}>
                                                Lat: {hasLocation ? myRestaurant?.autoLocation.coordinates[0].toFixed(6) : '--'}
                                            </Text>
                                        </View>
                                        <View className="px-3 py-2 border rounded-full" style={{ backgroundColor: AUTH_COLORS.chip, borderColor: AUTH_COLORS.chipBorder }}>
                                            <Text className="text-xs font-semibold" style={{ color: AUTH_COLORS.textPrimary }}>
                                                Lng: {hasLocation ? myRestaurant?.autoLocation.coordinates[1].toFixed(6) : '--'}
                                            </Text>
                                        </View>
                                    </View>
                                </View>
...
export default UpdateRestaurantDialogBox;